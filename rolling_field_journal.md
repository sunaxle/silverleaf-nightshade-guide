# 📓 Rolling Field Journal: Silverleaf Nightshade Expedition
**Project:** *Solanum elaeagnifolium* Statewide Collection & Ecology  
**Lead Researchers:** Dr. Sotti Lab & Field Team  
**Master Log File:** `rolling_field_journal.md`  

---

## 📅 Entry 1: iNaturalist Historical Data Mining & Hotspot Identification
* **Date/Time:** July 28, 2026 | 12:05 PM CDT  
* **Location Focus:** San Angelo, TX & Tom Green County  
* **Summary:**
  * Processed 72 historical iNaturalist observations of *Solanum elaeagnifolium* (98.6% Research Grade).
  * Determined peak phenology/blooming window in West Texas occurs in **April & May** (62.5% of observations).
  * Mapped primary cluster hotspots: San Angelo State Park, Grape Creek (US-87 N), W Houston Harte Frontage Rd, Civic League Park, and Middle Concho River.
  * Generated datasets: [san_angelo_inaturalist_obs.csv](file:///Users/dr3/Documents/Antigravity%20Designs/work/Silverleaf%20NightShade/san_angelo_inaturalist_obs.csv) and [san_angelo_inaturalist_obs.json](file:///Users/dr3/Documents/Antigravity%20Designs/work/Silverleaf%20NightShade/san_angelo_inaturalist_obs.json).

---

## 📅 Entry 2: Field Logistics, Permitting Protocols & Tube Preparation
* **Date/Time:** July 28, 2026 | 12:40 PM – 1:39 PM CDT  
* **Location:** En route to San Angelo, TX  
* **FedEx Destination Hub:** 3371 Knickerbocker Rd, San Angelo, TX 76904 (6:00 PM Shipping Cutoff)  
* **Key Field Decisions & Protocols:**
  1. **Pre-Labeling Protocol:** Tube bases and caps pre-marked with fine-tip Sharpie using standard nomenclature `[MMDD]-[SITE]-[TREATMENT]-[PLANT]-[TYPE]` (e.g., `0728-S01-D-P01-GEN`). Ensures leaf samples enter dry ice within 60 seconds of clipping without RNA degradation.
  2. **Permitting & Legal Shift:** Verified state park restrictions—harvesting/clipping in Texas State Parks requires a TPWD Scientific Study Permit. Shifted physical collection (genetic leaf clips, insect vials, root/soil bags) to **public highway rights-of-way (TxDOT shoulders and county road easements)** outside park fencelines.

---

## 📅 Entry 3: Active Collection Site #1 – Red Prairie Cluster
* **Date/Time:** July 28, 2026 | 1:47 PM CDT  
* **GPS Coordinates:** `31.42987, -100.47173` (Accuracy: 4m, Geoprivacy: Open)  
* **Proximity to FedEx:** 0.94 Miles (< 3 minutes from 3371 Knickerbocker Rd)  
* **Ecoregion / Soil Classification:** Red Prairie (US EPA Level IV Ecoregion) / Rolling Plains Ecoregion  
* **Habitat & Site Notes:**
  * Reddish-brown clay loam soil; roadside fence-line interface with high-density *Solanum elaeagnifolium* patch confirmed on iNaturalist.
  * Active flowering stems observed; low-to-moderate leaf damage scores.
* **Assigned Site & Sample Identifiers:**
  * **Site ID:** `S01` (San Angelo Site 1)
  * **Genetic Leaf Tube (Dry Ice):** `0728-S01-D-P01-GEN`
  * **Insect Vial:** `0728-S01-D-P01-BUG`
  * **Root & Soil Ziploc:** `0728-S01-D-P01-ROOTSOIL`

---

## 📅 Entry 4: Field Photo Micro-Grid Geolocation & Tube Replicate Mapping
* **Date/Time:** July 28, 2026 | 5:07 PM CDT  
* **Photo Folder Analyzed:** `7.28.26` (Desktop) — 38 Items (`IMG_9210` to `IMG_9250`)  
* **Spatial Resolution & Cluster Area:** `31.42967° N, -100.47624° W` (~290m × 100m micro-grid area)  
* **Replicate Mapping Breakdown:**
  * **Plant 1 (P01) — Photos #01–#07:** Habit shots, leaf close-ups, harvest.
  * **Plant 2 (P02) — Photos #08–#17:** Herbivory score, video pan, harvest.
  * **Plant 3 (P03) — Photos #18–#20:** Spine density close-ups, tube flash freeze.
  * **Plant 4 (P04) — Photos #21–#25:** Habitat overview, video site pan, harvest.
  * **Plant 5 (P05) — Photos #26–#32:** Flower macro, final harvest.
  * **Data Verification — Photos #33–#38:** Return spot photos, cooler check, Data Collection App screenshots.

---

## 📅 Entry 5: Post-Sampling Expedition Debrief & Operational Lessons
* **Date/Time:** July 28, 2026 | 5:25 PM CDT  
* **Summary of Operations & Wins:**
  * Established a strong rhythm balancing harvesting with macro photos.
  * Screened all plants for matching phenology (flowers + fruit concurrently present).
  * Maintained distance thresholds between plants to prevent sampling rhizomal clones.
  * Adapted to mowing disturbance by relocating past the trail edge to a mature mesquite (*Prosopis glandulosa*) canopy boundary.
* **Operational Bottlenecks Identified (~25–35 mins total time loss):**
  * **Dry Ice Slurry Prep:** Temp verification delay (<-70°C).
  * **Ink Running:** Double-bagged tubes in Ziplocs to protect Sharpie text from ethanol/condensation.
  * **Truck Transit:** Extra walking trips for tools/bags.
  * **Peak Heat Stress (2:30 PM – 4:00 PM):** High thermal load; bugs retreated into shade/soil, though foliar damage remained heavy.

---

## 📅 Entry 6: Real Yield Audit & Protocol Calibration for Dr. Alexis Racelis
* **Date/Time:** July 28, 2026 | 5:35 PM CDT  
* **Actual Tuesday Yield Audit:** 1 Site (`S01`), 3 Plant Replicates (`P01`, `P02`, `P03`).
* **Dry Ice Cooler Status:** Replicates 1, 2, and 3 are **currently stabilized on dry ice in the cooler**. Shipping was held today to combine with Wednesday's batch.
* **Protocol Scope Finding:** Combining Dr. Kariyat's above-ground protocol with Dr. Sotti's below-ground shovel digging triples sampling time per site (~1.5+ hrs).
* **P.I. Communication Strategy:** Posed 3 core questions to Dr. Alexis Racelis regarding scope streamlining, replicate count thresholds (3 vs 5), and Wednesday transit routing.
* **Full Memo File:** Link to [strategic_memo_dr_racelis.md](file:///Users/dr3/.gemini/antigravity/brain/828fa471-7f91-45b7-b6b6-a931aae6672e/strategic_memo_dr_racelis.md).

---

## 📅 Entry 7: Live Insect Colony Protocol & Option 1 Approval
* **Date/Time:** July 28, 2026 | 8:41 PM CDT  
* **Strategic Decisions:**
  1. **Option 1 Approved:** Wednesday schedule will execute 1 morning site in San Angelo (7:30–9:00 AM) → 3.5 hr transit to Wichita Falls hotel → 1 afternoon site in Wichita Falls.
  2. **Insect Survey & Colony Founding:** Focus shifted to capturing live *Leptinotarsa texana* / *L. defecta* adults and egg masses to establish lab colonies. Live bugs stored in ventilated tubes with damp paper and fresh foliage in a 18–22°C cool-box (never dry ice!).
  3. **Streamlined 3-Replicate Protocol:** Target 3 replicates per site (`P01`, `P02`, `P03`) to trim site time from 90 mins down to 20–25 mins.
  4. **Dr. Manish Kariyat Outreach:** Message drafted to confirm 3 replicates/site statistical sufficiency.

---

## 📅 Entry 8: Opportunistic Highway Sampling Protocol ('Sotti-Style' Roadside Stops)
* **Date/Time:** July 28, 2026 | 8:48 PM CDT  
* **Opportunistic Sampling Protocol Added:**
  * Enabled 5–10 minute "Sotti-Style" roadside opportunity stops during the highway drive when dense, high-herbivory *Solanum elaeagnifolium* colonies are spotted.
  * **Primary Focus:** Rapid insect plucking into live colony ventilated tubes (with fresh host leaves) and ethanol vials.

---

## 📅 Entry 9: Wednesday Route Revision — Abilene Sampling Stop Included
* **Date/Time:** July 28, 2026 | 9:08 PM CDT  
* **Route Update:** Added **Abilene, TX (Taylor County)** as a primary sampling stop on Wednesday.
* **Geographic Corridor:** San Angelo (US-277 N, 89 mi) → Abilene (Site `S03`, 10:15–11:15 AM) → Wichita Falls (US-277/US-287 N, 142 mi, Site `S04`, 3:45–5:00 PM). Total driving: ~231 miles (3.5 hours).
* **Ecoregion Benefit:** Captures 3 distinct ecoregion datasets in a single day: Red Prairie (San Angelo) → Rolling Plains/Limestone (Abilene) → Red River Basin (Wichita Falls).

---

## 📅 Entry 10: Physical Tube Labeling Database Integration (`SA01`)
* **Date/Time:** July 28, 2026 | 9:21 PM CDT  
* **Physical Tube Markings Verified:**
  * **Tube 1 (`R1`):** Cap `SA01-FLOW-R1` \| Side `SE | SA01 | FLOW | R1 (P1-P3) | 2026-07-28 | DR`
  * **Tube 2 (`R2`):** Cap `SA01-FLOW-R2` \| Side `SE | SA01 | FLOW | R2 (P4-P6) | 2026-07-28 | DR`
  * **Tube 3 (`R3`):** Cap `SA01-FLOW-R3` \| Side `SE | SA01 | FLOW | R3 (P7-P9) | 2026-07-28 | DR`
* **Master Mapping Updated:** Linked all physical tube cap/side strings to `photo_metadata.json`, `photo_lab.html`, and `photo_mapping_and_labeling_master.md`.

---

## 📅 Entry 11: Entomological Identification — *Gratiana pallidula* (Eggplant Tortoise Beetle)
* **Date/Time:** July 28, 2026 | 9:37 PM CDT  
* **Species Confirmed:** *Gratiana pallidula* (Chrysomelidae: Cassidinae)  
* **Key Observations & Morphological Evidence:**
  1. **Fecal Shields:** Dark, clumped structures held over larval backs via a hinged caudal furca (anal fork) for chemical/physical defense against ants and stink bugs.
  2. **Windowpane Herbivory:** Circular semi-translucent spots caused by early-instars scraping the upper epidermis while preserving the lower epidermal membrane.
  3. **Voucher Photo:** Mapped directly to Photo #12 (`IMG_9222.HEIC`).

---

## 📅 Entry 12: Integrated Woolly Nightshade Lace Bug (*Gargaphia decoris*) & Biocontrol Resource
* **Date/Time:** July 28, 2026 | 9:48 PM CDT  
* **Species Added:** Woolly Nightshade / Solanaceous Lace Bug (*Gargaphia decoris* Drake, Tingidae).
* **Biocontrol Documentation:** Integrated Manaaki Whenua Landcare Research biocontrol guide into Bug Guide web app (`index.html` / `app.js` / `data.js`).
* **Field Scouting Protocol:** Look on **underside of leaves (abaxial surface)** for gregarious spiny nymph clusters, adults (~3-4mm lace wings), pale yellow stippling, and black tar-like frass drops.

---

## 📅 Entry 13: Finalized Wednesday Expedition Blueprint (San Angelo → Abilene → Wichita Falls)
* **Date/Time:** July 28, 2026 | 10:07 PM CDT  
* **Finalized Itinerary:**
  1. **7:30 AM – 9:00 AM (San Angelo):** Sample 1–2 spots in San Angelo (`S02`). Whole-plant bagging + 3 fast replicates + live colony capture.
  2. **9:15 AM (FedEx Dropoff):** Consolidate Tuesday (`SA01`) + Wednesday morning (`S02`) dry ice tubes; drop at FedEx (3371 Knickerbocker Rd).
  3. **9:30 AM – 10:50 AM (Transit):** Drive US-277 N to Abilene (89 mi).
  4. **11:00 AM – 11:45 AM (Abilene):** Sample Site `S03` (Taylor County / Rolling Plains). Whole-plant bagging + live colony capture.
  5. **12:00 PM – 2:15 PM (Transit):** Drive US-277/US-287 N to Wichita Falls (142 mi). Make 5-min "Sotti-style" roadside opportunity stops (`OPP01`, `OPP02`).
  6. **2:30 PM – 3:30 PM (Hotel Check-In & Rest):** Check into Wichita Falls hotel, store live insect colony tubes in AC room, rehydrate team.
  7. **3:45 PM – 5:00 PM (Wichita Falls):** Sample Site `S04` (Red River Basin).
  8. **5:15 PM (FedEx Dropoff):** Drop afternoon dry ice batch at Wichita Falls FedEx store before 6:00 PM cutoff.
* **Full Plan File:** Link to [expedition_plan_and_insect_colony_sop.md](file:///Users/dr3/.gemini/antigravity/brain/828fa471-7f91-45b7-b6b6-a931aae6672e/expedition_plan_and_insect_colony_sop.md).

---

*This journal will be continuously updated as new sites are visited and samples are logged.*
